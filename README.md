...
The main script now stops on errors, waits for the container to become ready, lets you choose between `local-lvm` and `local-zfs` storage, prompts for a DNS server so the container can resolve package mirrors, automatically picks the latest available Debian 12 template (with a fallback if discovery is unavailable), and can optionally create a non-root admin user with passwordless sudo.
...
3. Fill in the prompts for your LXC spec, including bridge, DNS server, storage backend, disk, RAM, swap, and CPU settings
